...
import ClipLoader from "react-spinners/ClipLoader"; // For a nice loading spinner
  ...
  useEffect(() => {
    if (user?.email) {
      fetch(`https://sport-equipment-server-six.vercel.app/items/${user.email}`)
        ...
        .catch(() => setLoading(false)); // Stop loading even if an error occurs
    }
  }, [user?.email]);
        ...
        fetch(`https://sport-equipment-server-six.vercel.app/items/${id}`, {
          ...
          .then((data) => {
            if (data.deletedCount > 0) {
              ...
              setItem((prevItems) => prevItems.filter((it) => it._id !== id));
            }
          });
      }
    });
        ...
        {loading ? (
          ...
          <div className="text-center py-10 flex flex-col items-center">
            ...
            <p className="text-xl text-gray-500 mt-4">
              No equipment found! Add your first item now.
            </p>
            <Link to="/addItem" className="mt-4 inline-block">
              <button className="flex items-center gap-2 bg-blue-500 text-white px-5 py-2 rounded-lg hover:bg-blue-600">
                <IoIosAddCircle className="text-2xl" /> Add Item
              </button>
            </Link>
          </div>
        ) : (
